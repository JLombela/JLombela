...
export function KYCDetailsModal({ application, isOpen, onClose }: KYCDetailsModalProps) {
  ...
  const getRiskColor = (risk: string) => {
    switch (risk?.toLowerCase()) {
      case "low":
        return "border-axalio-green text-axalio-green"
      case "medium":
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl bg-popover border-border text-popover-foreground max-h-[90vh] overflow-y-auto">
        <DialogHeader className="flex flex-row items-center justify-between pb-4 border-b border-border">
          <div>
            ...
            <p className="text-muted-foreground font-mono text-sm mt-1">{application.type} COMPLIANCE APPLICATION</p>
          </div>
          <Button
            variant="ghost"
            size="sm"
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="bg-muted/50 border border-border p-4 rounded">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-xl font-bold font-mono text-popover-foreground">
                    {application.applicant || application.name}
                  </h3>
                  <p className="text-muted-foreground font-mono text-sm">{application.type} APPLICATION</p>
              ...
              <div className="space-y-3 text-sm font-mono">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-popover-foreground">{application.id}</span>
                </div>

                <div className="flex items-center gap-2">
                  <User className="w-4 h-4 text-muted-foreground" />
                  <span className="text-muted-foreground">TYPE:</span>
                  <span className="text-popover-foreground">{application.type}</span>
                </div>

                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-muted-foreground" />
                  <span className="text-muted-foreground">SUBMITTED:</span>
                  <span className="text-popover-foreground">{application.submittedDate}</span>
                </div>

                <div className="flex items-center gap-2">
                  <User className="w-4 h-4 text-muted-foreground" />
                  <span className="text-muted-foreground">REVIEWED BY:</span>
                  <span className="text-popover-foreground">{application.reviewedBy}</span>
                </div>

                <div className="flex items-center gap-2">
                  <FileText className="w-4 h-4 text-muted-foreground" />
                  <span className="text-muted-foreground">DOCUMENTS:</span>
                  <span className="text-popover-foreground">{application.documents}</span>
                </div>

                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-muted-foreground" />
                  <span className="text-muted-foreground">COMPLETION:</span>
                  <span className="text-popover-foreground">{application.completionRate}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-muted/50 border border-border p-4 rounded">
              ...
              <div className="space-y-2 text-sm font-mono">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-popover-foreground">+1 (555) 123-4567</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">LOCATION:</span>
                  <span className="text-popover-foreground">New York, USA</span>
                </div>
                {application.type === "KYB" && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">INDUSTRY:</span>
                    <span className="text-popover-foreground">Mining & Metals</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-muted/50 border border-border p-4 rounded">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground font-mono text-sm">CURRENT STATUS:</span>
                  <Badge className={`font-mono text-xs ${getStatusColor(application.status)}`}>
                    {application.status === "UNDER_REVIEW" ? "UNDER REVIEW" : application.status}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground font-mono text-sm">COMPLETION:</span>
                  <span className="text-popover-foreground font-mono text-sm">{application.completionRate}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground font-mono text-sm">DOCUMENTS:</span>
                  <span className="text-popover-foreground font-mono text-sm">{application.documents} FILES</span>
                </div>
              </div>
            </div>
                ...
                {documents.slice(0, application.documents).map((doc, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <FileText className="w-4 h-4 text-muted-foreground" />
                      <div>
                        <span className="text-popover-foreground font-mono text-sm">{doc.name}</span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        className="text-muted-foreground hover:text-popover-foreground p-1"
                      >
            ...
            <div className="bg-muted/50 border border-border p-4 rounded">
              ...
              <div className="space-y-3 max-h-48 overflow-y-auto">
                {reviewHistory.map((review, index) => (
                  <div key={index} className="p-3 bg-accent rounded border border-border">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      ></div>
                      <span className="text-muted-foreground font-mono text-xs">{review.date}</span>
                      <span className="text-muted-foreground font-mono text-xs">- {review.reviewer}</span>
                      {review.action === "DOCUMENTS_VERIFIED" && <CheckCircle className="w-3 h-3 text-axalio-green" />}
                    ...
                    <p className="text-popover-foreground font-mono text-sm">{review.note}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-muted/50 border border-border p-4 rounded">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-axalio-green" />
                  <span className="text-popover-foreground font-mono text-sm">Identity Verification</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-axalio-green" />
                  <span className="text-popover-foreground font-mono text-sm">Address Verification</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-axalio-green" />
                  <span className="text-popover-foreground font-mono text-sm">Business Registration</span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-popover-foreground font-mono text-sm">Financial Verification</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-axalio-green" />
                  <span className="text-popover-foreground font-mono text-sm">AML Screening</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-axalio-green" />
                  <span className="text-popover-foreground font-mono text-sm">Sanctions Check</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-border">
          ...
          <Button className="bg-axalio-green hover:bg-axalio-green/90 text-black font-mono">
